$(function(){
	window.defaultWebsites = 
	{
			"websites":
			[
				{"unique":"1", "title":"百度", "address":"http://m.baidu.com"},
				{"unique":"2", "title":"网易", "address":"http://www.163.com"},
				{"unique":"3", "title":"新浪", "address":"http://www.sina.com.cn"},
				{"unique":"4", "title":"CSDN", "address":"http://www.csdn.com"},
				{"unique":"5", "title":"优酷视频", "address":"http://www.youku.com"}
			]
	};
	...
	var readLocalData = function(){
		var buildDom = function(json){
			var i = 0;
			var html;
			var jqDomTarget = $(".website a.new").parent();
			while(i<json.websites.length){
				html = '<div class="website sortable">\
	...
	var websiteBindEvent = function() {
		...
		//编辑
		$(".website a.edit").unbind().click(function(){
		...
		//删除
		$(".website a.delete").unbind().click(function(){
			var jqWebsiteDel = $(this).parents(".website");
			$.noty.closeAll();
			//操作确认
			noty({type:"confirm", text: "确认删除该网址？", modal:true, layout:"center",
				buttons: [
	...
	$("#search-wrapper ul li").click(function(){
		...
		if(chooice=="baidu") {
			...
			$("#engine").attr("src", "img/logo_bing.png").attr("title", "bing");
		}
	...
	var goSearch = function(){
		...
		if(engine=="baidu") {
			...
			searchUrl = "http://www.bing.com/search?hl=zh-CN&source=hp&q="+keyWord;
		}
	...
	//新增网址
	$(".website a.new").click(function(){
		var newWebsite = $(this).parent();
		var addWebSite = function(){
			var title = arguments[0];
			var address = arguments[1];
	    	//UI更新
	    	var insertNewWebsite = '<div class="website sortable">\
	...
	//鼠标拖拽排序
	$("#website-container").sortable({cursor: "move", items: ".sortable", update: function(event, ui){
		//保存更新数据
		updateLocalData();
	}});
